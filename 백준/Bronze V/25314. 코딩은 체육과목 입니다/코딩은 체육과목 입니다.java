...
//		문제
//		오늘은 혜아의 면접 날이다. 
//		면접 준비를 열심히 해서 앞선 질문들을 잘 대답한 혜아는 
...
//		혜아가 N바이트 정수까지 저장할 수 있다고 생각하는 정수 자료형의 이름을 출력하여라.
		...
		int N; // 정수형 변수 N 선언 (N범위 설정을 위한)
		boolean isValid; // 불린 타입 변수 isValid 선언(반복문 종료를 위한)
		int fourNum; // 정수현 변수 fourNum 선언 (long 반복 횟수를 위한)
		
		// -------- 1) 반복문 시작 --------
		do {
			N = Integer.parseInt(br.readLine()); // BufferedReader를 통해 받아온 문자열을 정수형으로 형변환 후 N에 대입
			isValid = (N >= 4 && 1000 >= N && N % 4 == 0); // isValid 조건 설정(1)
			if (!isValid) {
				System.out.println("N 은 4 ~ 1000중 4의 배수만 들어갈 수 있습니다");
			}
			fourNum = N / 4;
		} while (!isValid);
		// -------- 1) 반복문 종료 --------
		...
		for(int i = 0; i < fourNum; i++ ) {
			System.out.print("long "); // fourNum 숫자만큼 long 과 공백 출력
		}
		// -------- 2) 반복문 종료 -------- 
		
		System.out.print("int"); // 마지막 int 출력
